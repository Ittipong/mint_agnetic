        ...
        async for stream_mode, chunk in graph.astream(
            ...
            if stream_mode == "custom":
                ...
                if isinstance(chunk, dict) and "validator_failed" in chunk:
                    ...
                    continue
                # Follow-up suggestions are about to be generated (a separate
                # ~0–4s LLM call inside the gen_suggestions node). Forward a
                # lightweight, payload-free signal so the client can show a
                # "fetching follow-ups" placeholder during the gap before the
                # `suggestions` block arrives. Only fired on chip-producing
                # turns (the node gates first), so the client never shows it
                # on a turn that yields no chips.
                if isinstance(chunk, dict) and chunk.get("suggestions_pending"):
                    yield {"event": "suggestions_pending", "data": ""}
                    continue
