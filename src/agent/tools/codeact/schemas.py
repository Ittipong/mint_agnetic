"""Pydantic models for the analyze pipeline (CodeAct).

Ported from v2 (`codeact_subgraph/shared/schemas.py`) in Wave 1 —
relocated under `src/agent/tools/codeact/` because v3 collapses the
CodeAct sub-graph into a single `run_python` tool.
