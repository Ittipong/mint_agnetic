...
- `restaurant_bill` → exactly 1 expense transaction for the WHOLE meal — do NOT split per dish. Sum every food/drink line plus service charge and VAT into one `amount` equal to the grand total, pick the single food/dining category that best fits the meal, and set `note` to the restaurant name. A diner reads one restaurant bill as one "ค่าอาหาร" expense, not a basket of ingredients.
- `retail_receipt` → one transaction per item line (shopping basket stays itemised); VAT only when added-on-top; discount as income with `include_in_report=false`.
