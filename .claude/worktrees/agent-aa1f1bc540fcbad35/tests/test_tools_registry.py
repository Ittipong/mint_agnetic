...
def test_UT_T00_ALL_TOOLS_has_exactly_six_entries():
    """UT-T00: the tool registry must expose EXACTLY 7 tools.
    ...
    so follow-up chips are guaranteed on non-ADD/non-crisis turns.
    ...
    assert len(ALL_TOOLS) == 7, (
        f"expected 7 tools (emit_suggestions demoted to the `suggest` node), "
...
def test_UT_T00_expected_tool_names_match_phase2_spec():
    """The 7 tools must match phase2_tools_design.md exactly — names are the
