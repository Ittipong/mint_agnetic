...
async def lifespan(app: FastAPI):
    ...
    # 5b. Long-term-memory store (factory handles None / missing dep).
...
async def _persist_slip_proposal(
    ...
    # `as_node="finalize"` is MANDATORY — see docstring + memory above.
    await g.aupdate_state(cfg, {"proposals": existing}, as_node="finalize")
...
async def _finalize_proposal_in_state(
    ...
    *,
    ...
    # `as_node="finalize"` — see docstring + memory `project_slip_vision_as_node`.
    await g.aupdate_state(cfg, update, as_node="finalize")
