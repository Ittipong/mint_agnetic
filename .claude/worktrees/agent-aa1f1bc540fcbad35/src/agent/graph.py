...
def _make_prompt(state: AgentState) -> list:
    ...
    system_text = render_system_prompt(today=today_iso, user_id=user_id)
...
def _pre_turn_hook(state: AgentState) -> dict:
    """Clear per-turn scratch + inject `__repo__` (Issue 2).
...
async def build_graph(
    *,
    ...
    Args:
      ...
      store:               LangGraph BaseStore (from `utils.store_factory`)
                           for cross-thread memory tools. None disables
                           `memory_recall` / `memory_write` cleanly.
