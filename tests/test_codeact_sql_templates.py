...
def test_UT_SQL01_sum_expense_emits_expected_shape():
    ...
    # The SQL must restrict to confirmed, undeleted rows.
    assert "t.is_deleted = false" in sql
    assert "t.status = 'confirmed'" in sql
    # include_in_report must NOT filter rows — totals count every transaction.
    assert "include_in_report" not in sql
...
# ── UT-SQL-CB — sum_by_category no longer feeds a per-category card ─────────
# The category_breakdown card was retired (breakdowns render as a table/bullet
# list inside the answer again). The SQL must NOT pull the category `icon`
# JSONB nor LEFT JOIN `categories cat` — that join existed ONLY for the card.
# `category_sync_id` (from the transactions table) stays: prose still uses it
# for the `{{cat:<sync_id>}}` inline-icon token in bullet-list breakdowns.


def test_UT_SQL_CB01_sum_by_category_drops_icon_join():
    """UT-SQL-CB01: build_query for sum_by_category must NOT LEFT JOIN
    `categories cat` nor select the icon JSONB — the card that needed it is
    gone. category_sync_id remains for the bullet-list icon token."""
    spec = _bare("sum_by_category")
    sql, _ = build_query(spec, user_id="u-1")
    assert "LEFT JOIN categories cat" not in sql
    assert "cat.icon" not in sql
    assert "category_sync_id" in sql


def test_UT_SQL_CB02_sum_by_category_thb_branch_drops_icon_join():
    """UT-SQL-CB02: the convert_to_thb branch also drops the icon select + join,
    while the currencies join (alias `c`) is untouched."""
    spec = _bare("sum_by_category", convert_to_thb=True)
    sql, _ = build_query(spec, user_id="u-1")
    assert "LEFT JOIN categories cat" not in sql
    assert "cat.icon" not in sql
    # currencies join still uses alias `c`.
