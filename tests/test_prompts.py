...
#   (c) a key returned by a non-SQL tool (get_user_context /
...
# non-SQL tools (get_user_context, propose_transaction's
...
_PYTHON_COMPOSITE_KEYS: frozenset[str] = frozenset({
    ...
    # spend_for — category-vs-note resolution result
    "mode", "term", "keyword", "type",
    "cat_total", "cat_cnt", "cat_txns",
    "note_total", "note_cnt", "note_txns",
...
def test_UT_P06_prompt_forbids_confirmation_without_propose_call():
    ...
    confirmation ("ขอยืนยัน…" / "กดยืนยัน") is only valid AFTER a
    ...
    # confirmation ("กดยืนยัน") — never claim the save already happened.
    # The card's position ("ด้านล่าง"/"ด้านบน") must NOT be named.
    assert "กดยืนยันเพื่อบันทึก" in SYSTEM_PROMPT
    # The confirmation copy must not pin the card to a position — only the
    # negative guard ('Do NOT name the position …') may mention the words.
    assert "กดยืนยันด้านล่าง" not in SYSTEM_PROMPT
    assert "กดยืนยันด้านบน" not in SYSTEM_PROMPT
